...
public static partial class RuptureDetector
{
    ...
    [GeneratedRegex(@"\b(nie\s*słucha|nie\s*slucha|nie\s*pomag|don.?t\s*understand|you.?re\s*not\s*listening|stop\s*it|frustruj|annoying|ignor|to\s+nie\s+pomaga|męczy\s+mnie\s+to|meczy\s+mnie\s+to|bez\s+sensu\s+ta\s+rozmowa|gadam\s+jak\s+do\s+ściany|gadam\s+jak\s+do\s+sciany)",
        RegexOptions.IgnoreCase, matchTimeoutMilliseconds: 100)]
