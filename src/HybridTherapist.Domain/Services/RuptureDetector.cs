...
public static partial class RuptureDetector
{
    ...
    [GeneratedRegex(@"\b(nie|źle|zle|wcale|wręcz|wrecz|nie\s*to|nie\s*tak|nie\s*zrozumiał|zrozumial|nie\s*chodziło|chodzilo|nie\s*o\s*to|źle\s*mnie\s*rozumiesz|wrong|that.?s\s*not|no,?\s*i\s*don.?t|misunderstood|missed\s*the\s*point|not\s*what\s*i)\b",
        RegexOptions.IgnoreCase, matchTimeoutMilliseconds: 100)]
    private static partial Regex CorrectionSignals();

    [GeneratedRegex(@"\b(nie\s*słucha|nie\s*slucha|nie\s*pomag|don.?t\s*understand|you.?re\s*not\s*listening|stop\s*it|frustruj|annoying|ignor)",
        RegexOptions.IgnoreCase, matchTimeoutMilliseconds: 100)]
